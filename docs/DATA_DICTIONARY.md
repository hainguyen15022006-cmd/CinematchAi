...
pseudo-text rules). Manifest schema 1.1 separates exact-file integrity from
reproducibility; dataset version and the 167-dimensional feature contract are
unchanged. Loaders reject incompatible versions when callers pass
`expected_data_version` and `expected_feature_contract_version`; the coverage
CLI passes both from the configuration.

### Integrity versus reproducibility

- `artifacts.<name>.sha256` hashes every byte of the file. Use it to detect
  corruption or changes to that exact exported file, including timestamps.
- `artifacts.<name>.content_sha256` follows `cinematch-content-v1`: for JSON,
  remove only the root-level `generated_at_utc`, sort object keys and serialize
  compact UTF-8 JSON. Array order, nested timestamps, numeric values and all
  other fields remain significant. Invalid JSON is rejected. For non-JSON
  files (CSV/NPZ), this is the exact-file SHA-256; there is no float rounding.
- `reproducibility.content_sha256` hashes the named artifact content hashes
  together with dataset/feature versions, feature-generation parameters,
  split policy, quality statistics and evaluation contract. It excludes
  generation times, file paths, file sizes and exact-file hashes.

Compare the canonicalization policy and the complete bundle fingerprint
between machines. Different JSON creation times/formatting do not change it;
changed data, features, seeds or generation rules do. If it differs, compare
per-artifact content hashes and the configuration fields to locate the cause.
Different numeric output bytes still count as a mismatch and must be
investigated, not hidden with rounding. The fingerprint identifies a bundle;
it does not certify model quality or replace version validation.
...
| `artifacts` | Relative paths, sizes, row counts, exact-file `sha256` and `content_sha256` |
| `feature_generation` | Seed, positive threshold and pseudo-text generation rules |
| `reproducibility` | Hash algorithm, canonicalization policy and stable bundle fingerprint |
