...
def create_room(db: Session, host_id: int, name: str = None) -> Room:
...
def join_room(db: Session, code: str, user_id: int) -> Room:
    ...
    existing_member = db.query(RoomMember).filter(RoomMember.room_id == room.id, RoomMember.user_id == user_id).first()
